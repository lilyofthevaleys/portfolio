...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // Batch all reads first to prevent layout thrashing
    const cardData = cardsRef.current.map((card, i) => {
      if (!card) return null;
      const cardTop = getElementOffset(card);
      return { card, cardTop, index: i };
    }).filter(Boolean) as { card: HTMLElement; cardTop: number; index: number }[];

    // Then do all calculations and writes
    cardData.forEach(({ card, cardTop, index: i }) => {
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardData.length; j++) {
          const jCardTop = cardData[j].cardTop;
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // Use native window scroll instead of Lenis to avoid conflicts
